-- `building-an-mcp-server` topic's fixed quiz shell (TR+EN) -- SORU İÇERMİYOR, aynı desen
...
-- Quiz'e OTOMATİK EKLEMEZ -- `building-an-mcp-server` için hiç bir quiz satırı yoktu, bu
...
SELECT id, 'tr', 'default', 'Bilgini Test Et', 0.80, true FROM topic WHERE slug = 'building-an-mcp-server';

INSERT INTO quiz (topic_id, language, slug, title, pass_threshold, active)
SELECT id, 'en', 'default', 'Test Your Knowledge', 0.80, true FROM topic WHERE slug = 'building-an-mcp-server';
